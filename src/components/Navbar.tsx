...
const Navbar = () => {
  ...
  return (
    <nav className=" sticky top-0 z-50 bg-gray-200 dark:bg-black/90 backdrop-blur-md border-b border-slate-300 dark:border-slate-700 transition-colors" lang={currentLanguage}>
     ...
     <div className="flex justify-between items-center h-24">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                to={item.path}
                onClick={scrollToTop}
                className={` px-2 py-1 rounded-lg font-medium relative transition-all duration-200 ${
                    isActive(item.path)
                ...
                <span className={isImpact ? "impact-animated-border-inner" : ""}>
                  ...
                  {isImpact && (
                    <span className="absolute -top-3 -right-3 flex items-center justify-center pointer-events-none">
                      <FaStar
                        className={` text-yellow-400 drop-shadow-md transition-transform duration-300 
                                      ${isStarAnimating ? "impact-star-blink" : ""} `}
                        style={{ fontSize: "1.3em", zIndex: 10 }}
                      />
                      <span className="absolute inset-0 rounded-full bg-yellow-400 opacity-50 animate-ping-impact" style={{ zIndex: 1 }} />
                    </span>
                  )}
                </span>
              </Link>
      ...
      <button 
        onClick={toggleTheme} 
        className="p-1 rounded-full border border-slate-300 md:mr-2 dark:border-slate-600 text-black dark:text-white transition"
      >
